...
وأسعار الخطط **بعملة بلد الزائر حسب الـ IP** لكل **١٩٥ دولة** (أسعار صرف حيّة)، مع **ضرائب كل دولة اختيارية**
...
> ✅ **آخر تحقق شامل (r24 — سبتمبر 2026):** لاندينج **Global AI Business OS** عالمية فاخرة (موكاب لوحة حي بعملة قابلة للتبديل + شريط تحكم عالمي تفاعلي + عرض AI بمراجعة قبل التنفيذ + باقة 10$ واحدة) وبوابة دخول SaaS — E2E عربي/إنجليزي/جوال 390px بلا overflow، دخول فعلي وتطبيق سليمان، lint نظيف. التفاصيل في [منجز حديثاً](#-منجز-حديثاً).
...
| 🔐 بوابة الدخول | `#/login` | **بوابة SaaS (r24):** نموذج الدخول/التسجيل/الاستعادة + معاينة حية للوحة بعملة قابلة للتبديل + نقاط ثقة + لغة/عملة/سمة أعلى + شريط «مجاني لأول 100 شركة» أسفل — و`#/login?mode=register` يفتح تبويب التسجيل مباشرة (وجهة زر «ابدأ مجاناً») |
...
| الاشتراكات | `GET /api/pricing` (عام: الخطط بعملة بلد الزائر حسب الـ IP + ١٩٥ دولة + VAT) · `GET/PUT/POST /api/subscription` (لوحة «حسابي»: البروفايل + اللغة + البلد + الاستخدام + طلب ترقية) · `GET/PUT/POST /api/admin/subscriptions` (لوحة المؤسس: الخطط + المشتركون + الطلبات — مدير فقط) |
...
| `Client` | `clients` | دليل العملاء المحفوظ | `name` · `phone` · `email` · `address` · `country` · `governorate` · `company` |
...
| البند | الجولة |
|---|---|
| **🌏 سلايدر «مرحباً بالعالم» بكل لغات العالم (r23)**: تحية الموقع الرئيسية صارت سلايدراً لا نهائياً بأسلوب إعلانات آبل داخل قسم البطل (مكان السلايدر، لا عند فتح الموقع): «مرحباً بالعالم» تتعاقب بـ**77 لغة** بخطّ يناسب كل نظام كتابة (يدوي للاتينية · سيريف للغات الأخرى · CJK) + اسم اللغة وعدّاد تحت التحية بلغة الواجهة + تبدأ السلسلة بلغة الزائر + إيقاف مؤقت عند التحويم/اللمس أو إخفاء التبويب + تراعي prefers-reduced-motion — والعنوان الحقيقي h1 يبقى للـ SEO | r23 |
| **🗺️ محافظات ومقاطعات كل دول العالم + فلسطين (r23)**: بطاقة العميل صارت عالمية: منتقي **الدولة (195 دولة — فلسطين 🇵🇸 بلا إسرائيل)** بأعلام ومجمّعات إقليمية + منتقي **المحافظة/المقاطعة المتتالي** بـ**4870 تقسيماً إدارياً** لكل الدول — الدول العربية الـ22 (وبها فلسطين بـ16 محافظة) بأسماء عربية كاملة بترتيب منطقي والباقي بأسماء إنجليزية — التخزين باسم أساسي إنجليزي مع عرض عربي/إنجليزي حسب لغة الواجهة · عمودان جديدان في تصدير واستيراد CSV/Excel بتطبيع ذكي (عربي/إنجليزي/كود ISO → أساس) · الجدول يعرض العلم والمحافظة والدولة · تعبئة عنوان الفاتورة تلقائياً بالمحافظة والدولة عند غيابه · البحث في الدليل يشمل الدولة والمحافظة — البيانات مولّدة بـ`scripts/build-world-states.cjs` (حزمة country-state-city + تراكب عربي يدوي) في `src/lib/world-states.ts` كـchunk ديناميكي | r23 |
| **🤖 تحسينات الواجهة (r23)**: فقاعة Garfix AI العائمة بتلميح عند التحويم («اسأل Garfix AI ✨») وتفاعل لمسي/بصري + تذييل التطبيق الجديد (الشركة · Garfix · المؤسس) + ترجمات جديدة للقاموس الإنجليزي | r23 |
...
| **🌍 السيستم بكل لغات وعملات العالم**: ٢٨ لغة (منتقي لغات حيّ باتجاه RTL/LTR تلقائي) + ١٩٥ دولة بعملاتها (أسعار صرف حيّة من open.er-api.com بكاش Valkey ٦ ساعات + أرقام احتياطية) + **ضرائب كل دولة اختيارية** (VAT قياسية لكل بلد — تفعيل ونسبة افتراضية من إعدادات الشركة وتعديل لكل فاتورة) | r18 |
