...
- 💬 **المساعد الذكي**: شات ببث SSE حقيقي متصل بلقطة حيّة من كامل بيانات المشروع (المؤشرات، أعلى المديونيات، أحدث الفواتير، عينة الكتالوج، آخر التذكيرات) — يجيب بالأرقام الفعلية بعملة الشركة النشطة
...
| الذكاء الاصطناعي | `POST /api/ai/chat` (SSE) · `POST /api/ai/action` (تنفيذ إجراء مقترح) · `GET/PUT /api/ai/config` · `POST /api/ai/test` · `GET/DELETE /api/ai/conversations` · `GET /api/ai/conversations/[id]` · `POST /api/ai/process-items` |
...
> **39 ملف مسار** في `src/app/api` (تحقّق مباشر). العمليات الإدارية الحساسة (الشركات، إعداد/اختبار الذكاء، النسخ الاحتياطي/الاستعادة، دمج العملاء، محتوى الموقع، طوابير المهام) تتطلب **جلسة مدير** على الخادم — القائمة الكاملة في [قسم الأمان](#-الأمان-security).
