...
import { SITE_CSS, DEFAULT_CONTENT, LogoMark } from "./site-shared";
 ...
 *   #/founder → رسالة المؤسس #/login → بوابة الدخول/التسجيل (r24)
 * r16: #/reset?token=… → إعادة تعيين كلمة المرور (من رسالة Resend)
 * r24: الهوية العالمية «GarfiX — AI Business OS» (نافبار/فوتر بأيقونات lucide).
 */
...
function SiteInner({ page = "home", authed = false, onEnterApp = () => {} }) {
  ...
  // بوابة الدخول (r24): نموذج + معاينة حية + لغة/عملة أعلى + أول 100 شركة أسفل
  if (page === "login") {
    return (
  ...
  return (
    <div className="site-root">
      ...
      <div className="s-z" style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
        {/* ── Navbar (r24: مكوّن قابل لإعادة الاستخدام بأيقونات lucide) ── */}
        <PublicNavbar page={page} authed={authed} onEnterApp={enter} onNav={navHash} content={content} />
        ...
        <footer className="s-footer">
          <div style={{ maxWidth: 1180, margin: "0 auto", padding: "34px 20px 18px" }}>
            <div className="s-footer-cols">
              <div>
                ...
                <p style={{ color: "rgba(255,255,255,.5)", fontSize: 12.5, lineHeight: 1.9, margin: 0, maxWidth: 340 }}>
                  {tr("GarfiX — نظام تشغيل ذكي للفواتير والمدفوعات وإدارة الشركات: تعدد الشركات والعملات، المدفوعات الجزئية، وتذكيرات واتساب — لأي شركة في أي دولة.")}
                </p>
              </div>
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {[
                    { hash: "#/", label: t("nav.home") },
                    { hash: "#/pricing", label: t("nav.pricing") },
                    { hash: "#/team", label: t("nav.team") },
                    { hash: "#/founder", label: t("nav.founder") },
                  ].map((l) => (
                    <a key={l.hash} href={l.hash} onClick={(e) => { e.preventDefault(); navHash(l.hash); }} style={{ color: "rgba(255,255,255,.6)", textDecoration: "none", fontSize: 13 }}>
                      {l.label}
                    </a>
                  ))}
                  {!authed && (
                    <a href="#/login?mode=register" onClick={(e) => { e.preventDefault(); navHash("#/login?mode=register"); }} style={{ color: "rgba(255,255,255,.6)", textDecoration: "none", fontSize: 13 }}>
                      {tr("إنشاء حساب مجاني")}
                    </a>
                  )}
                </div>
              ...
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8, color: "rgba(255,255,255,.6)", fontSize: 13 }}>
                  {content.contact_phone && (
                    <a href={`tel:${content.contact_phone.replace(/\s/g, "")}`} style={{ color: "inherit", textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 8 }}>
                      <Phone size={13.5} color="#c9a227" aria-hidden="true" /> <span dir="ltr" className="s-num">{content.contact_phone}</span>
                    </a>
                  )}
                  {content.contact_email && (
                    <a href={`mailto:${content.contact_email}`} style={{ color: "inherit", textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 8 }}>
                      <Mail size={13.5} color="#c9a227" aria-hidden="true" /> {content.contact_email}
                    </a>
                  )}
                  {content.contact_address && (
                    <span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
                      <MapPin size={13.5} color="#c9a227" aria-hidden="true" /> {tr(content.contact_address)}
                    </span>
                  )}
                </div>
              </div>
