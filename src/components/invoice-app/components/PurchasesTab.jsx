...
export default function PurchasesTab({ company, invoices = [], preSelectIds = [], onClearPreSelect }) {
  ...
  const card = { background: "var(--ia-card)", borderRadius: "12px", boxShadow: "0 1px 4px rgba(0,0,0,.08)", border: "1px solid var(--ia-border)" };
  const inp = { width: "100%", border: "1.5px solid var(--ia-border2)", borderRadius: "8px", padding: "9px 12px", fontFamily: "inherit", fontSize: "13px", background: "var(--ia-card)", outline: "none" };
  const lbl = { fontSize: "11px", color: "var(--ia-sub)", fontWeight: 700, display: "block", marginBottom: "5px" };
  ...
  return (
    <div style={{ fontFamily: "'Cairo','Tajawal',sans-serif", direction: "rtl" }}>
          ...
          {purchases.map(pi => {
            ...
            return (
              <div key={pi.id} style={{ ...card, overflow: "hidden" }}>
                ...
                {expanded && (
                  <div style={{ borderTop: "1px solid var(--ia-border3)", overflowX: "auto" }}>
                      ...
                      <tbody>
                        {(pi.items || []).map((r, i) => (
                          <tr key={i} style={{ borderBottom: "1px solid var(--ia-border3)", background: i % 2 === 0 ? "var(--ia-card)" : "var(--ia-row-alt)" }}>
                            <td style={{ padding: "9px 14px", color: "var(--ia-muted)" }}>{i + 1}</td>
                            <td style={{ padding: "9px 14px", fontWeight: 700 }}>{r.name}</td>
                            <td style={{ padding: "9px 14px", textAlign: "center", fontWeight: 900, fontSize: "15px", color: colTx }}>{r.qty}</td>
                            <td style={{ padding: "9px 14px", color: r.purchasePrice ? "var(--ia-red-tx)" : "var(--ia-muted)" }}>
      ...
      {/* Generate Modal */}
      {showModal && (
        <div style={{ position: "fixed", inset: 0, background: "var(--ia-overlay)", zIndex: 2000, display: "flex", alignItems: "stretch", justifyContent: "center", padding: "0" }}
          onClick={() => setShowModal(false)}>
          <div style={{ width: "100%", maxWidth: "680px", margin: "auto", background: "var(--ia-card)", borderRadius: "16px", display: "flex", flexDirection: "column", maxHeight: "94vh", overflow: "hidden" }}
              ...
              <div style={{ marginBottom: "16px" }}>
                ...
                <div style={{ maxHeight: "220px", overflowY: "auto", border: "1px solid var(--ia-border)", borderRadius: "8px" }}>
                  {filtered.length === 0 ? (
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
                      <thead>
                        <tr style={{ background: "var(--ia-soft)", position: "sticky", top: 0 }}>
                        ...
                        {filtered.map(inv => {
                          ...
                          return (
                            <tr key={inv.id} onClick={() => toggleSel(inv.id)}
                              style={{ cursor: "pointer", background: isSel ? `${col}0a` : "", borderBottom: "1px solid var(--ia-border3)" }}>
                              <td style={{ padding: "7px 10px", textAlign: "center" }}>
                                <input type="checkbox" checked={isSel} onChange={() => {}} style={{ cursor: "pointer", accentColor: col }} />
                              </td>
                              <td style={{ padding: "7px 10px" }}>
                                <span style={{ background: "var(--ia-blue-bg)", color: "var(--ia-blue-tx)", borderRadius: "20px", padding: "1px 7px", fontSize: "11px", fontWeight: 700 }}>{inv.invNum}</span>
                              </td>
                              <td style={{ padding: "7px 10px", fontWeight: 600, fontSize: "12px" }}>{inv.clientName}</td>
                              <td style={{ padding: "7px 10px", color: "var(--ia-sub)", fontSize: "11px" }}>{fDate(inv.date)}</td>
                              <td style={{ padding: "7px 10px", color: "var(--ia-sub)", fontSize: "11px" }}>
                      ...
                      <tbody>
                        {merged.map((r, i) => (
                          <tr key={i} style={{ borderBottom: "1px solid var(--ia-border3)", background: i % 2 === 0 ? "var(--ia-card)" : "var(--ia-row-alt)" }}>
                            ...
                            <td style={{ padding: "8px 12px", textAlign: "center", fontWeight: 900, fontSize: "15px", color: colTx }}>{r.qty}</td>
