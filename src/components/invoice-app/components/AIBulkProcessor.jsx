...
const CONF_COLOR = c =>
  c >= 0.85 ? "#16a34a" : c >= 0.65 ? "#d97706" : "#dc2626";
const CONF_LABEL = c =>
  c >= 0.85 ? "عالية" : c >= 0.65 ? "متوسطة" : "منخفضة";
const CONF_BG = c =>
  c >= 0.85 ? "#dcfce7" : c >= 0.65 ? "#fef3c7" : "#fee2e2";
...
export default function AIBulkProcessor({ company, onPurchaseSaved }) {
  ...
  // ── Styles ────────────────────────────────────────────────────────
  const s = {
    card: { background: "#fff", borderRadius: "12px", border: "1px solid #e5e7eb", boxShadow: "0 1px 4px rgba(0,0,0,.07)" },
    ...
    inp: { width: "100%", border: "1.5px solid #d1d5db", borderRadius: "8px", padding: "8px 11px", fontFamily: "inherit", fontSize: "13px", outline: "none" },
    lbl: { fontSize: "11px", color: "#6b7280", fontWeight: 700, display: "block", marginBottom: "4px" },
    sec: { fontSize: "11px", fontWeight: 900, color: "#6b7280", textTransform: "uppercase", letterSpacing: ".5px", marginBottom: "10px" },
  };
    ...
    <div style={{ fontFamily: "'Cairo','Tajawal',sans-serif", direction: "rtl" }}>
      ...
      {step === 0 && (
        <div style={{ ...s.card, padding: "20px" }}>
          ...
          <div style={{ background: "#f8fafc", border: "1px solid #e5e7eb", borderRadius: "8px", padding: "11px 14px", marginBottom: "12px", fontSize: "12px", color: "#6b7280", lineHeight: "1.7" }}>
            <b style={{ color: "#374151" }}>أمثلة:</b> <span dir="ltr">3 m19 headphones, ٥ شواحن ايفون, 2 powerbank 20k, اربع سماعات JBL, cable type c × 10</span>
          </div>
          <textarea
            ref={textRef}
          ...
          />
          {error && (
            <div style={{ background: "#fee2e2", border: "1px solid #fca5a5", borderRadius: "8px", padding: "10px 14px", color: "#dc2626", fontSize: "13px", marginBottom: "12px" }}>
              ❌ {error}
            </div>
          )}
      ...
      {step === 1 && (
        <div style={{ ...s.card, padding: "56px", textAlign: "center" }}>
          ...
          <div style={{ color: "#6b7280", fontSize: "13px" }}>يتم تحليل النص واستخراج المنتجات وتطبيعها</div>
      ...
      {step === 2 && (
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          ...
          <div style={{ ...s.card, padding: "14px 18px", display: "flex", gap: "16px", flexWrap: "wrap", alignItems: "center" }}>
            <div>
              <span style={{ fontSize: "12px", color: "#6b7280" }}>المنتجات المكتشفة</span>
              <div style={{ fontWeight: 900, fontSize: "22px", color: col }}>{aiItems.length}</div>
            </div>
            <div>
              <span style={{ fontSize: "12px", color: "#6b7280" }}>بعد الدمج</span>
              <div style={{ fontWeight: 900, fontSize: "22px", color: "#7c3aed" }}>{merged.length}</div>
            </div>
            <div>
              <span style={{ fontSize: "12px", color: "#6b7280" }}>إجمالي الكميات</span>
              <div style={{ fontWeight: 900, fontSize: "22px", color: "#16a34a" }}>{merged.reduce((s, it) => s + it.quantity, 0)}</div>
            </div>
            <div style={{ flex: 1 }} />
            <button style={s.btn("#e5e7eb", "#374151")} onClick={() => { setStep(0); setAiItems([]); }}>← إعادة الإدخال</button>
          </div>

          {/* Individual AI Results */}
          <div style={{ ...s.card, overflow: "hidden" }}>
            <div style={{ padding: "14px 18px 8px", borderBottom: "1px solid #f0f0f0" }}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
                <thead>
                  <tr style={{ background: "#f8fafc" }}>
                    <th style={{ padding: "9px 12px", textAlign: "right", color: "#6b7280", fontWeight: 700, fontSize: "11px" }}>النص الأصلي</th>
                    <th style={{ padding: "9px 12px", textAlign: "right", color: "#6b7280", fontWeight: 700, fontSize: "11px" }}>الاسم المطبّع</th>
                    <th style={{ padding: "9px 12px", textAlign: "center", color: "#6b7280", fontWeight: 700, fontSize: "11px", width: "80px" }}>الكمية</th>
                    <th style={{ padding: "9px 12px", textAlign: "right", color: "#6b7280", fontWeight: 700, fontSize: "11px", width: "120px" }}>سعر الشراء KD</th>
                    <th style={{ padding: "9px 12px", textAlign: "center", color: "#6b7280", fontWeight: 700, fontSize: "11px", width: "90px" }}>الثقة</th>
                    <th style={{ padding: "9px 12px", width: "40px" }}></th>
                  </tr>
                </thead>
                <tbody>
                  {aiItems.map((it) => (
                    <tr key={it._id} style={{ borderBottom: "1px solid #f5f5f5", background: it.edited ? "#fefce8" : "#fff" }}>
                      <td style={{ padding: "8px 12px", color: "#9ca3af", fontSize: "12px", maxWidth: "140px" }}>
                        <div style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{it.rawText}</div>
                        {it.catalogName && (
                          <div style={{ color: "#7c3aed", fontSize: "11px", marginTop: "2px" }}>🔗 {it.catalogName}</div>
                        )}
                      </td>
                      ...
                      <td style={{ padding: "8px 12px", textAlign: "center" }}>
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "2px" }}>
                          <span style={{ background: CONF_BG(it.confidence), color: CONF_COLOR(it.confidence), borderRadius: "20px", padding: "2px 8px", fontSize: "11px", fontWeight: 700 }}>
                            {CONF_LABEL(it.confidence)}
                          </span>
                          <span style={{ color: "#9ca3af", fontSize: "10px" }}>{Math.round(it.confidence * 100)}%</span>
                        </div>
                      </td>
      ...
      {step === 3 && (
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          ...
          {/* Catalog list */}
          <div style={{ ...s.card, overflow: "hidden" }}>
            <div style={{ padding: "14px 18px 8px", borderBottom: "1px solid #f0f0f0", display: "flex", alignItems: "center", gap: "8px" }}>
              <div style={s.sec}>قائمة المنتجات في الكتالوج</div>
              <div style={{ flex: 1 }} />
              <span style={{ fontSize: "12px", color: "#6b7280" }}>{catalog.length} منتج</span>
            </div>
            {catalogLoading ? (
              <div style={{ padding: "24px", textAlign: "center", color: "#9ca3af" }}>⏳ جارٍ التحميل...</div>
            ) : catalog.length === 0 ? (
              ...
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
                  <thead>
                    <tr style={{ background: "#f8fafc" }}>
                      <th style={{ padding: "9px 14px", textAlign: "right", color: "#6b7280", fontWeight: 700, fontSize: "11px" }}>الاسم</th>
                      <th style={{ padding: "9px 14px", textAlign: "right", color: "#6b7280", fontWeight: 700, fontSize: "11px" }}>الأسماء البديلة</th>
                      <th style={{ padding: "9px 14px", textAlign: "right", color: "#6b7280", fontWeight: 700, fontSize: "11px" }}>الشراء KD</th>
                      <th style={{ padding: "9px 14px", textAlign: "right", color: "#6b7280", fontWeight: 700, fontSize: "11px" }}>البيع KD</th>
          ...
          <button style={s.btn("#e5e7eb", "#374151")} onClick={() => setStep(0)}>← رجوع للإدخال</button>
        </div>
      )}
      ...
      {step === 4 && (
        <div style={{ ...s.card, padding: "56px", textAlign: "center" }}>
          ...
          <div style={{ fontSize: "17px", fontWeight: 900, marginBottom: "8px", color: "#16a34a" }}>تم حفظ فاتورة المشتريات!</div>
...
function CatalogRow({ prod, i, s, col, onDelete, onUpdate, editing, onEdit, onCancelEdit }) {
      ...
      <tr style={{ borderBottom: "1px solid #e5e7eb", background: "#fefce8" }}>
        ...
        <td style={{ padding: "6px 8px" }}>
          <div style={{ display: "flex", gap: "4px" }}>
            ...
            <button onClick={onCancelEdit} style={{ background: "#e5e7eb", border: "none", borderRadius: "6px", padding: "4px 8px", cursor: "pointer", fontSize: "12px" }}>✕</button>
          </div>
        </td>
      </tr>
    );
  ...
  return (
    <tr style={{ borderBottom: "1px solid #f5f5f5", background: i % 2 === 0 ? "#fff" : "#fafafa" }}>
      <td style={{ padding: "9px 14px", fontWeight: 700 }}>{prod.name}</td>
      <td style={{ padding: "9px 14px", fontSize: "12px", color: "#6b7280" }}>
      ...
      <td style={{ padding: "9px 8px" }}>
        <div style={{ display: "flex", gap: "4px" }}>
          <button onClick={onEdit} style={{ background: "#fef3c7", color: "#92400e", border: "none", borderRadius: "6px", padding: "4px 8px", cursor: "pointer", fontSize: "12px" }}>✏️</button>
          <button onClick={() => onDelete(prod.id)} style={{ background: "#fee2e2", color: "#dc2626", border: "none", borderRadius: "6px", padding: "4px 8px", cursor: "pointer", fontSize: "12px" }}>🗑️</button>
        </div>
      </td>
    </tr>
  );
