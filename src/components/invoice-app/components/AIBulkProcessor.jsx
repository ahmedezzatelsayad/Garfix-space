...
const CONF_LABEL = c =>
  c >= 0.85 ? tr("عالية") : c >= 0.65 ? tr("متوسطة") : tr("منخفضة");
...
export default function AIBulkProcessor({ company, onPurchaseSaved }) {
  ...
  const processWithAI = async () => {
    ...
    try {
      ...
      setError(e.message || tr("حدث خطأ في المعالجة"));
      setStep(0);
    }
  };
  ...
  const savePurchase = async () => {
    ...
    try {
      ...
      toast_("❌ " + (e.message || tr("حدث خطأ أثناء الحفظ")));
  ...
  const addProduct = async () => {
    ...
    try {
      ...
      toast_(tr("✅ تم إضافة المنتج للكتالوج"));
  ...
  const updateCatalogItem = async (id, patch) => {
    try {
      ...
      toast_(tr("✅ تم التحديث"));
    } catch { toast_(tr("❌ فشل التحديث")); }
  };
  ...
  return (
    <div style={{ fontFamily: "'Cairo','Tajawal',sans-serif", direction: "rtl" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "16px", flexWrap: "wrap" }}>
        <div style={{ fontSize: "17px", fontWeight: 900, color: col }}>{tr("🤖 معالجة AI للمشتريات")}</div>
        <div style={{ flex: 1 }} />
        <button style={s.btn("#7c3aed")} onClick={() => setStep(step === 3 ? 0 : 3)}>
          📦 {step === 3 ? tr("رجوع") : tr("كتالوج المنتجات ({0})",[catalog.length])}
        </button>
      </div>
      ...
      {step === 0 && (
        <div style={{ ...s.card, padding: "20px" }}>
          <div style={s.sec}>{tr("أدخل قائمة المنتجات (عربي أو إنجليزي أو مختلط)")}</div>
          <div style={{ background: "var(--ia-soft)", border: "1px solid var(--ia-border)", borderRadius: "8px", padding: "11px 14px", marginBottom: "12px", fontSize: "12px", color: "var(--ia-sub)", lineHeight: "1.7" }}>
            <b style={{ color: "var(--ia-text2)" }}>{tr("أمثلة:")}</b> <span dir="ltr">{tr("3 m19 headphones, ٥ شواحن ايفون, 2 powerbank 20k, اربع سماعات JBL, cable type c × 10")}</span>
          </div>
          <textarea
            ref={textRef}
            style={{ ...s.inp, minHeight: "160px", resize: "vertical", marginBottom: "12px", fontSize: "13px", lineHeight: "1.8" }}
            placeholder={tr("3 m19 headphones\n٥ شواحن ايفون\n2 powerbank 20k\nاربع سماعات JBL\ncable type c × 10")}
            value={rawText}
          ...
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
            <button style={{ ...s.btn(col), flex: 1, justifyContent: "center", padding: "12px" }}
              ...
              {tr("🤖 معالجة بالذكاء الاصطناعي")}
            </button>
            {catalog.length === 0 && (
              <button style={s.btn("#7c3aed")} onClick={() => setStep(3)}>
                {tr("📦 أضف كتالوج المنتجات أولاً")}
              </button>
            )}
          </div>
          {catalog.length > 0 && (
            <div style={{ marginTop: "10px", fontSize: "12px", color: "var(--ia-sub)" }}>
              {tr("🔍 سيتم المطابقة مع")} <b>{catalog.length}</b> {tr("منتج في الكتالوج")}
            </div>
          )}
        </div>
      )}
      ...
      {step === 1 && (
        <div style={{ ...s.card, padding: "56px", textAlign: "center" }}>
          ...
          <div style={{ fontSize: "16px", fontWeight: 700, color: col, marginBottom: "6px" }}>{tr("جارٍ المعالجة بالذكاء الاصطناعي...")}</div>
          <div style={{ color: "var(--ia-sub)", fontSize: "13px" }}>{tr("يتم تحليل النص واستخراج المنتجات وتطبيعها")}</div>
      ...
      {step === 2 && (
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          ...
          <div style={{ ...s.card, padding: "14px 18px", display: "flex", gap: "16px", flexWrap: "wrap", alignItems: "center" }}>
            <div>
              <span style={{ fontSize: "12px", color: "var(--ia-sub)" }}>{tr("المنتجات المكتشفة")}</span>
              <div style={{ fontWeight: 900, fontSize: "22px", color: col }}>{aiItems.length}</div>
            </div>
            <div>
              <span style={{ fontSize: "12px", color: "var(--ia-sub)" }}>{tr("بعد الدمج")}</span>
              <div style={{ fontWeight: 900, fontSize: "22px", color: "var(--ia-vio-tx)" }}>{merged.length}</div>
            </div>
            <div>
              <span style={{ fontSize: "12px", color: "var(--ia-sub)" }}>{tr("إجمالي الكميات")}</span>
              <div style={{ fontWeight: 900, fontSize: "22px", color: "var(--ia-ok-tx)" }}>{merged.reduce((s, it) => s + it.quantity, 0)}</div>
            </div>
            <div style={{ flex: 1 }} />
            <button style={s.btn("var(--ia-ghost-bg)", "var(--ia-ghost-tx)")} onClick={() => { setStep(0); setAiItems([]); }}>{tr("← إعادة الإدخال")}</button>
          </div>

          {/* Individual AI Results */}
          <div style={{ ...s.card, overflow: "hidden" }}>
            <div style={{ padding: "14px 18px 8px", borderBottom: "1px solid var(--ia-border3)" }}>
              <div style={s.sec}>{tr("نتائج الذكاء الاصطناعي — راجع وعدّل قبل الحفظ")}</div>
            </div>
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
                <thead>
                  <tr style={{ background: "var(--ia-soft)" }}>
                    <th style={{ padding: "9px 12px", textAlign: "start", color: "var(--ia-sub)", fontWeight: 700, fontSize: "11px" }}>{tr("النص الأصلي")}</th>
                    <th style={{ padding: "9px 12px", textAlign: "start", color: "var(--ia-sub)", fontWeight: 700, fontSize: "11px" }}>{tr("الاسم المطبّع")}</th>
                    <th style={{ padding: "9px 12px", textAlign: "center", color: "var(--ia-sub)", fontWeight: 700, fontSize: "11px", width: "80px" }}>{tr("الكمية")}</th>
                    <th style={{ padding: "9px 12px", textAlign: "start", color: "var(--ia-sub)", fontWeight: 700, fontSize: "11px", width: "120px" }}>{tr("سعر الشراء KD")}</th>
                    <th style={{ padding: "9px 12px", textAlign: "center", color: "var(--ia-sub)", fontWeight: 700, fontSize: "11px", width: "90px" }}>{tr("الثقة")}</th>
          ...
          {merged.length > 0 && (
            <div style={{ ...s.card, overflow: "hidden" }}>
              <div style={{ padding: "14px 18px 8px", borderBottom: "1px solid var(--ia-border3)" }}>
                <div style={s.sec}>{tr("📦 المنتجات المدمجة — هذا ما سيُحفظ في فاتورة المشتريات")}</div>
              </div>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
                <thead>
                  <tr style={{ background: col + "15" }}>
                    <th style={{ padding: "8px 14px", textAlign: "start", color: col, fontWeight: 700 }}>{tr("المنتج")}</th>
                    <th style={{ padding: "8px 14px", textAlign: "center", color: col, fontWeight: 700 }}>{tr("الكمية الكلية")}</th>
                    <th style={{ padding: "8px 14px", textAlign: "start", color: col, fontWeight: 700 }}>{tr("سعر الشراء")}</th>
                    <th style={{ padding: "8px 14px", textAlign: "start", color: col, fontWeight: 700 }}>{tr("الإجمالي")}</th>
                  </tr>
                </thead>
                <tbody>
                  {merged.map((it, i) => {
                ...
                {merged.reduce((s, it) => s + pN(it.purchasePrice) * it.quantity, 0) > 0 && (
                  <tfoot>
                    <tr style={{ background: col + "20" }}>
                      <td colSpan={3} style={{ padding: "10px 14px", fontWeight: 900, color: col }}>{tr("الإجمالي الكلي")}</td>
          ...
          <div style={{ ...s.card, padding: "16px 18px" }}>
            <div style={s.sec}>{tr("بيانات فاتورة المشتريات")}</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "12px" }}>
              <div>
                <label style={s.lbl}>{tr("اسم المورد")}</label>
                <input style={s.inp} placeholder={tr("اختياري")} value={purchaseForm.supplier}
                  onChange={e => setPurchaseForm(f => ({ ...f, supplier: e.target.value }))} />
              </div>
              <div>
                <label style={s.lbl}>{tr("التاريخ")}</label>
                <input style={s.inp} type="date" value={purchaseForm.date}
                  onChange={e => setPurchaseForm(f => ({ ...f, date: e.target.value }))} />
              </div>
            </div>
            <div style={{ marginBottom: "14px" }}>
              <label style={s.lbl}>{tr("ملاحظات")}</label>
              <input style={s.inp} placeholder={tr("اختياري")} value={purchaseForm.notes}
            ...
            <div style={{ display: "flex", gap: "8px" }}>
              <button style={{ ...s.btn("#7c3aed"), flex: 1, justifyContent: "center", padding: "12px", fontSize: "14px" }}
                ...
                {saving ? tr("جارٍ الحفظ...") : tr("💾 حفظ فاتورة المشتريات ({0} منتج)",[merged.length])}
              </button>
            </div>
          </div>
        </div>
      ...
      {step === 3 && (
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          {/* Add product */}
          <div style={{ ...s.card, padding: "16px 18px" }}>
            <div style={s.sec}>{tr("➕ إضافة منتج جديد للكتالوج")}</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "10px" }}>
              <div>
                <label style={s.lbl}>{tr("اسم المنتج *")}</label>
                <input style={s.inp} placeholder={tr("مثال: سماعة JBL")} value={newProduct.name}
                  onChange={e => setNewProduct(p => ({ ...p, name: e.target.value }))} />
              </div>
              <div>
                <label style={s.lbl}>{tr("أسماء بديلة (مفصولة بفاصلة)")}</label>
                <input style={s.inp} placeholder={tr("jbl, سماعات, headphone jbl")} value={newProduct.aliases}
                  onChange={e => setNewProduct(p => ({ ...p, aliases: e.target.value }))} />
              </div>
              <div>
                <label style={s.lbl}>{tr("سعر الشراء KD")}</label>
              ...
              <div>
                <label style={s.lbl}>{tr("سعر البيع KD")}</label>
            ...
            <button style={s.btn(col)} onClick={addProduct} disabled={addingProduct || !newProduct.name.trim()}>
              {addingProduct ? tr("جارٍ الإضافة...") : tr("➕ إضافة للكتالوج")}
            </button>
          </div>
          ...
          <div style={{ ...s.card, overflow: "hidden" }}>
            <div style={{ padding: "14px 18px 8px", borderBottom: "1px solid var(--ia-border3)", display: "flex", alignItems: "center", gap: "8px" }}>
              <div style={s.sec}>{tr("قائمة المنتجات في الكتالوج")}</div>
              <div style={{ flex: 1 }} />
              <span style={{ fontSize: "12px", color: "var(--ia-sub)" }}>{catalog.length} {tr("منتج")}</span>
            </div>
            {catalogLoading ? (
              <div style={{ padding: "24px", textAlign: "center", color: "var(--ia-muted)" }}>{tr("⏳ جارٍ التحميل...")}</div>
            ) : catalog.length === 0 ? (
              <div style={{ padding: "32px", textAlign: "center", color: "var(--ia-muted)" }}>
                <div style={{ fontSize: "32px", marginBottom: "8px" }}>📦</div>
                <div>{tr("الكتالوج فارغ — أضف منتجاتك أعلاه")}</div>
              </div>
            ) : (
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
                  <thead>
                    <tr style={{ background: "var(--ia-soft)" }}>
                      <th style={{ padding: "9px 14px", textAlign: "start", color: "var(--ia-sub)", fontWeight: 700, fontSize: "11px" }}>{tr("الاسم")}</th>
                      <th style={{ padding: "9px 14px", textAlign: "start", color: "var(--ia-sub)", fontWeight: 700, fontSize: "11px" }}>{tr("الأسماء البديلة")}</th>
                      <th style={{ padding: "9px 14px", textAlign: "start", color: "var(--ia-sub)", fontWeight: 700, fontSize: "11px" }}>{tr("الشراء KD")}</th>
                      <th style={{ padding: "9px 14px", textAlign: "start", color: "var(--ia-sub)", fontWeight: 700, fontSize: "11px" }}>{tr("البيع KD")}</th>
          ...
          <button style={s.btn("var(--ia-ghost-bg)", "var(--ia-ghost-tx)")} onClick={() => setStep(0)}>{tr("← رجوع للإدخال")}</button>
        </div>
      )}
      ...
      {step === 4 && (
        <div style={{ ...s.card, padding: "56px", textAlign: "center" }}>
          ...
          <div style={{ fontSize: "17px", fontWeight: 900, marginBottom: "8px", color: "var(--ia-ok-tx)" }}>{tr("تم حفظ فاتورة المشتريات!")}</div>
          <div style={{ color: "var(--ia-sub)", fontSize: "13px", marginBottom: "24px" }}>
            {merged.length} {tr("منتج ·")} {merged.reduce((s, it) => s + it.quantity, 0)} {tr("قطعة إجمالية")}
          </div>
