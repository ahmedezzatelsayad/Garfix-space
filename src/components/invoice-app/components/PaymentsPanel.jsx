...
export default function PaymentsPanel({ inv, company, onChanged, canEdit }) {
  ...
  const save = async () => {
    ...
    if (!amount || amount <= 0) { setErr(tr("أدخل مبلغاً صحيحاً أكبر من صفر")); return; }
    if (amount > remaining + 0.001) { setErr(tr("المبلغ يتجاوز المتبقي ({0})",[fKWD(remaining)])); return; }
    ...
    try {
      ...
      setErr(tr("تعذّر حفظ الدفعة — تحقق من الاتصال"));
    } finally {
      setSaving(false);
    }
  };
  ...
  const del = async (p) => {
    if (!confirm(tr("حذف دفعة {0}؟ سيتم خصمها من المبلغ المدفوع",[fKWD(p.amount)]))) return;
    try {
    ...
    <div className="card" style={{ overflow: "hidden", marginTop: "12px", animation: "fadeUp .25s" }}>
      ...
      <div style={{ background: `linear-gradient(135deg,${col} 0%,${col}dd 100%)`, color: "#fff", padding: "12px 18px", display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
        ...
        {canEdit && remaining <= 0 && payments.length > 0 && (
          <span style={{ fontSize: "11px", background: "rgba(255,255,255,.18)", padding: "3px 10px", borderRadius: "20px", fontWeight: 700 }}>{tr("✓ مدفوعة بالكامل")}</span>
        )}
      </div>
      ...
      {iT(inv) > 0 && (
        <div style={{ padding: "10px 18px 4px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: "11px", color: "var(--ia-sub)", marginBottom: "4px", fontWeight: 700 }}>
            <span>{tr("المدفوع:")} {fKWD(pN(inv.paid || 0))}</span>
            <span>{tr("المتبقي:")} {fKWD(remaining)}</span>
          </div>
      ...
      <div style={{ padding: "8px 18px 16px" }}>
        {loading ? (
          <div style={{ textAlign: "center", color: "var(--ia-muted)", padding: "18px 0", fontSize: "13px" }}>{tr("⏳ جارٍ تحميل الدفعات...")}</div>
        ) : payments.length === 0 ? (
          <div style={{ textAlign: "center", color: "var(--ia-muted)", padding: "18px 0", fontSize: "13px" }}>
            <div style={{ fontSize: "28px", marginBottom: "6px" }}>🧾</div>
            {tr("لا توجد دفعات مسجّلة")}{remaining > 0 ? tr(" — سجّل أول دفعة لهذه الفاتورة") : ""}
          </div>
        ) : (
            ...
            {payments.slice().sort((a, b) => (a.date < b.date ? 1 : -1)).map((p, i) => {
              ...
              return (
                <div key={p.id} style={{ display: "flex", alignItems: "center", gap: "10px", background: "var(--ia-row-alt)", border: "1px solid var(--ia-border3)", borderRadius: "9px", padding: "9px 12px" }}>
                  <span style={{ width: "34px", height: "34px", borderRadius: "9px", background: m.bg, display: "inline-flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{tr(m.label).split(" ")[0]}</span>
                  <div style={{ minWidth: 0, flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "7px", flexWrap: "wrap" }}>
                      <span style={{ fontWeight: 900, fontSize: "13.5px", color: "var(--ia-text)", direction: "ltr" }}>{fKWD(p.amount)}</span>
                      <span style={{ fontSize: "10.5px", background: m.bg, color: m.color, padding: "1px 8px", borderRadius: "20px", fontWeight: 800 }}>{tr(m.label).replace(/^\S+\s/, "")}</span>
                  ...
                  {canEdit && (
                    <button onClick={() => del(p)} title={tr("حذف الدفعة")} style={{ background: "transparent", border: "1px solid var(--ia-red-bd)", color: "var(--ia-red-tx)", borderRadius: "7px", padding: "4px 9px", fontFamily: "inherit", fontSize: "12px", cursor: "pointer", flexShrink: 0 }}>🗑️</button>
                  )}
                </div>
              );
            })}
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.5)", zIndex: 3000, display: "flex", alignItems: "center", justifyContent: "center", padding: "16px" }} onClick={() => setShowModal(false)}>
          <div className="card" style={{ width: "100%", maxWidth: "380px", padding: "22px 24px", animation: "fadeUp .2s" }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
              ...
              <div style={{ fontWeight: 900, fontSize: "15px", color: col }}>{tr("تسجيل دفعة")}</div>
              <div style={{ flex: 1 }} />
            ...
            <div className="form-2col" style={{ marginBottom: "12px" }}>
              <div>
                <label style={{ fontSize: "11px", color: "var(--ia-sub)", display: "block", marginBottom: "4px", fontWeight: 700 }}>{tr("التاريخ")}</label>
                <input className="inp" type="date" value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))} />
              </div>
              <div>
                <label style={{ fontSize: "11px", color: "var(--ia-sub)", display: "block", marginBottom: "4px", fontWeight: 700 }}>{tr("ملاحظة")}</label>
                <input className="inp" placeholder={tr("اختياري")} value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))} />
              </div>
            </div>
            ...
            <div style={{ display: "flex", gap: "8px" }}>
              <button className="btn" style={{ background: "#16a34a", color: "#fff", flex: 1, fontSize: "14px", padding: "10px" }} disabled={saving} onClick={save}>
                {saving ? tr("⏳ جارٍ الحفظ...") : tr("💾 حفظ الدفعة")}
              </button>
              <button className="btn btn-ghost" onClick={() => setShowModal(false)}>{tr("إلغاء")}</button>
            </div>
          </div>
        </div>
      )}
